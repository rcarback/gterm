...
///
/// A peer refuses the probe request rather than accepting it, because the probe
/// asks to cancel a forwarding that was never established. The refusal is still
/// a complete round-trip, so callers record it with `recordReply()`. Only a
/// total absence of any answer counts as a miss.
